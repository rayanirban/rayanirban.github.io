...
and whether generated samples remain consistent with the measurement model. When a forward model is known, student and teacher samples should be checked for consistency. In the general case, this is a likelihood-based residual:
...
1. **Sample mean.** Compare $\mathbb{E}_{p_{\phi}}[x \mid y]$ vs. $\mathbb{E}_{p_{\theta}}[x \mid y]$.

2. **Sample variance and uncertainty maps.** Compare $\operatorname{Var}_{p_{\phi}}(x \mid y)$ vs. $\operatorname{Var}_{p_{\theta}}(x \mid y)$.

3. **Task-specific biological statistics.** Compare the distributions of biologically relevant statistics:
...
4. **Measurement consistency.** Compare the likelihood-based residuals:
...
5. **Calibration.** Check whether nominal credible intervals or uncertainty estimates have the expected empirical coverage.

6. **Diversity and mode coverage.** Use ensemble statistics, distributional distances, or downstream biological measurements to check whether the student has collapsed to a subset of teacher-like outputs.
